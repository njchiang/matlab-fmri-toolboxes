function [smm_rs, smm_ps, n, searchlightRDMs] = parSearchlightMapping_fMRI(fullBrainVolumes, models, mask, userOptions, localOptions)
% written 8/20/2015-- fully parallelized
% ARGUMENTS
...
searchlightVecRDMs = nan([nConditions*(nConditions-1)/2, volSize_vox]);
if monitor
...
%             progressMonitor(cMappingVoxI, nVox_mappingMask_request, 'Searchlight mapping Mahalanobis distance...', h_progressMonitor);
    ...
    vec_n(cMappingVoxI)=length(cIllValidVox_YspaceINDs);
    if vec_n(cMappingVoxI) < 2, continue; end%if % This stops the function crashing if it accidentally encounters an out-of-brain floating voxel (these can occur if, for example, skull stripping fails)
    ...
    searchlightVecRDMs(:,cMappingVoxI)=searchlightRDM;
    try
...
searchlightRDMs = nan([nConditions, nConditions, volSize_vox]);
 % rewrite as normal sized
for cMappingVoxI=1:nVox_mappingMask_request
    [x, y, z]=ind2sub(volSize_vox,mappingMask_request_INDs(cMappingVoxI));
    smm_ps(x,y,z,:) = vec_ps(cMappingVoxI,:);
    smm_rs(x,y,z,:) = vec_rs(cMappingVoxI,:);
    n(x,y,z) = vec_n(cMappingVoxI);
    searchlightRDMs(:, :, x, y, z) = squareform(searchlightVecRDMs(:, cMappingVoxI));
end

clear vec_n vec_ps vec_rs searchlightVecRDMs
%% END OF THE BIG LOOP! %%
